...
class CliPropertiesTest {
  ...
  void defaults_areApplied_whenNoOverrides() {
    contextRunner.run(context -> {
      ...
      assertThat(props.getEndpoints().getIpGeoBaseUrl().toString()).isEqualTo("http://ip-api.com/json");
  ...
  void overrides_bindFromProperties() {
    contextRunner
        ...
        .run(context -> {
          ...
          assertThat(props.getEndpoints().getIpGeoBaseUrl().toString()).isEqualTo("https://example.com/ip");
          assertThat(props.getEndpoints().getOpenMeteoBaseUrl().toString()).isEqualTo("https://example.com/weather");
        });
  }
  ...
  void negativeTimeouts_failValidation() {
    contextRunner
      .withPropertyValues(
        "cli.network.connect-timeout-ms=0",
        "cli.network.read-timeout-ms=-1")
