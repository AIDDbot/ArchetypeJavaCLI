...
public class OpenMeteoClient {
  ...
  public WeatherObservation fetchCurrent(double lat, double lon) {
    try {
      ...
      String uri = String.format("%s?latitude=%s&longitude=%s&current_weather=true", baseUri.toString(), lat, lon);

      var wrapper = webClient.get()
          .uri(uri)
          .retrieve()
          .bodyToMono(OpenMeteoResponseWrapper.class)
          .block();
      ...
      if (cur.getTemperature() == null || cur.getWindspeed() == null || cur.getWeathercode() == null) {
        throw new CodedException(ExitCodes.UNKNOWN, "open-meteo returned incomplete current_weather");
      }
      ...
      throw new CodedException(ExitCodes.NETWORK, "open-meteo HTTP error: " + wcre.getMessage(), wcre);
    } catch (RuntimeException re) {
      log.error("open-meteo network/runtime error: {}", re.toString());
      Throwable unwrapped = Exceptions.unwrap(re);
      throw new CodedException(ExitCodes.NETWORK, "open-meteo network error: " + unwrapped.getMessage(), unwrapped);
    }
  }
